            ...
            onComplete: function(response) {
                if (response.responseText.isJSON()) {
                    ...
                    if (needUpdate && res.updatepayment) {
                        ...
                        var CheckOnepageCheckoutExists_for_payment = document.getElementById('onepagecheckout_orderform');
                        if (CheckOnepageCheckoutExists_for_payment != null && res.paymentblock)
                        {
                             var custompayment = $$('div#checkout-payment-method-load').first();
                               custompayment.className = "loading";
                               custompayment.update('');
                               setTimeout(function(){ custompayment.update(res.paymentblock); custompayment.className = "";}, 1000);
                ...
                /// ********** Updated code By Pws **********
                    var CheckOnepageCheckoutExists = document.getElementById('onepagecheckout_orderform');
                    if (CheckOnepageCheckoutExists != null)
                    {
                         var url_reload = 'https://yourdomain.com/onepagecheckout/index/updateCheckout/';
                        var summary = $$('div#checkout-review-load').first();
                       document.getElementById("checkout-review-load").className = "loading";
                       
                       
                        summary.update('');
                        new Ajax.Request(url_reload, {
                        method: 'post',
                        parameters: 'payment-method=0&review=1',
                        onComplete: function(transport) {
                            if (transport.responseText.isJSON()) {
                                var response = transport.responseText.evalJSON();
                                console.log(response);
                              document.getElementById("checkout-review-load").className = "";
                                summary.update(response.update_section.review);
                                    }
                                }
                            });
...
function getElement() {
    if ($('checkout-payment-method-load123').down('#checkout-payment-method-load123') == undefined) {
        return  $('checkout-payment-method-load123');
    } else {
        return $('checkout-payment-method-load123').down('#checkout-payment-method-load123');
    }
}